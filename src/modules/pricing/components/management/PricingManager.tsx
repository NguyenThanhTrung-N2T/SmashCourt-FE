...
import { ManageBranchCourtTypesModal } from "../dialogs/ManageBranchCourtTypesModal";
import { PricingHeader } from "../layout/PricingHeader";
import { PricingStats } from "../stats/PricingStats";
import { PricingTabs } from "../layout/PricingTabs";
import { PricingGridTab } from "../layout/PricingGridTab";
        ...
        <div className="space-y-6 animate-slide-up w-full px-8 pt-6 pb-10">
            ...
            {tab === "branch" && (
                <div className="flex items-center gap-3 bg-surface-1 p-4 rounded-2xl border border-border shadow-sm">
                    ...
                    <div className="flex-1">
                        <p className="text-xs font-bold text-muted uppercase tracking-wide">Chọn chi nhánh ghi đè</p>
            ...
            {tab === "branch" && !selectedBranchId ? (
                <div className="flex flex-col items-center justify-center py-20 text-center bg-surface-1 rounded-4xl border border-border shadow-sm">
                    ...
                    <p className="text-sm text-muted mt-1 max-w-sm">Chọn một chi nhánh từ danh sách trên để xem hoặc cấu hình bảng giá ghi đè.</p>
                </div>
            ) : resolving ? (
                <GridSkeleton />
            ) : (
            ...
            >
                <div
                    className={`flex items-center gap-3 rounded-2xl border-2 bg-surface-1 px-5 py-4 shadow-2xl ${toast.tone === "success" ? "border-emerald-500/40" : "border-red-500/40"
                        }`}
                >
                    {toast.tone === "success" ? (
