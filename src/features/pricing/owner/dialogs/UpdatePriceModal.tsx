...
export function UpdatePriceModal({ row, onClose, onSave }: UpdatePriceModalProps) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-md rounded-3xl bg-surface-1 shadow-2xl border border-border overflow-hidden animate-slide-up">
                {/* Header */}
                <div
                ...
                <div className="p-6 space-y-5">
                    ...
                    <div>
                        ...
                        <div className="space-y-2">
                            {[
                                ...
                                >
                                    <div
                                        className={`h-4 w-4 rounded-full border-2 shrink-0 flex items-center justify-center transition-all ${applyAll === opt.value ? "border-primary bg-primary" : "border-border"
                                            }`}
                                    >
                                        {applyAll === opt.value && (
                                            <div className="h-1.5 w-1.5 rounded-full bg-surface-1" />
                                        )}
                                    </div>
                                    <div>
                        ...
                        {applyAll && (
                            <div className="mt-3 rounded-xl bg-amber-500/10 border border-amber-500/30 p-3 flex items-start gap-2 animate-slide-up">
                        ...
                        <div>
                            ...
                            <div className="relative">
                                <input
                                    ...
                                    className="w-full rounded-xl border border-border bg-surface-2 pl-4 pr-8 py-2.5 text-sm font-bold text-foreground focus:outline-none focus:border-primary focus:ring-4 focus:ring-primary/10"
                                />
                                <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs font-bold text-muted">đ</span>
                            </div>
                        </div>
                        <div>
                            <label className="block text-xs font-bold text-foreground mb-1.5 uppercase tracking-wide">
                    ...
                    <div className="rounded-2xl bg-surface-2 border border-border p-4">
                        <p className="text-xs font-bold text-muted uppercase tracking-wide mb-2">Xem trước thay đổi</p>
                        <div className="grid grid-cols-2 gap-3 text-center">
                            <div>
                                <p className="text-[10px] text-muted font-semibold uppercase">Ngày thường</p>
                                <p className="text-base font-extrabold text-muted line-through">{fmt(row.weekdayPrice)}</p>
                                <p className="text-lg font-extrabold text-primary">
                                    {Number(weekday) > 0 ? fmt(Number(weekday)) : "–"}
                                </p>
                            </div>
                            <div>
                                <p className="text-[10px] text-muted font-semibold uppercase">Cuối tuần</p>
                                <p className="text-base font-extrabold text-muted line-through">{fmt(row.weekendPrice)}</p>
                                <p className="text-lg font-extrabold text-primary">
                ...
                <div className="px-6 pb-6 flex justify-end gap-3">
                    <button
                        onClick={onClose}
                        className="rounded-xl border border-border bg-surface-1 px-5 py-2.5 text-sm font-bold text-foreground hover:bg-surface-2 transition-colors"
                    >
                        Hủy
                    </button>
                    <button
