...
import type { CourtType, PriceConfig, PriceRow, PriceVersionDto } from "../../../types";
...
interface PricingGridTabProps {
    ...
    onResetOverride?: (row: PriceRow) => void;
    fallbackVersionDate?: string;
}
        ...
        return (
            <div className="flex flex-col items-center justify-center py-24 text-center bg-surface-1 rounded-4xl border border-border shadow-sm overflow-hidden relative">
                ...
                <p className="text-sm text-muted mt-2 max-w-sm relative z-10">
                    Bạn cần chọn ít nhất 1 loại sân từ hệ thống để bắt đầu thiết lập bảng giá cho chi nhánh này.
                </p>
            </div>
        );
    }
        ...
        <div className="rounded-4xl border border-border bg-surface-1 shadow-sm overflow-hidden">
            <div className="border-b border-border bg-surface-2 px-6 py-4 flex items-center justify-between flex-wrap gap-3">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        {!currentConfig && <p className="text-xs text-muted">Chưa có bảng giá</p>}
                    </div>
                </div>

                {(hasVersions || fallbackVersionDate) && (
                    <div className="flex items-center gap-3">
                        <div className="relative">
                            <select
                                ...
                                className="appearance-none rounded-xl border border-border bg-surface-1 px-4 py-2 pr-10 text-sm font-bold text-foreground outline-none transition-colors hover:border-primary focus:border-primary focus:ring-2 focus:ring-primary/20 disabled:cursor-not-allowed disabled:opacity-70"
                            >
                                {hasVersions ? (
                                    versions.map((version, index) => {
                                        const versionNumber = versions.length - index; // oldest = 1, current = N
                                        const label = version.isCurrent ? "Phiên bản hiện tại" : `Phiên bản ${versionNumber}`;
                                        return (
                                            <option key={version.effectiveFrom} value={version.effectiveFrom}>
                                                {label} (từ {fmtDate(version.effectiveFrom)})
                                            </option>
                                        );
                                    })
                                ) : (
                                    <option value={fallbackVersionDate}>
                                        Hôm nay - Giá hệ thống ({fmtDate(fallbackVersionDate ?? "")})
                                    </option>
                                )}
                            </select>
                        ...
                        <button
                            onClick={onRefresh}
                            className="flex items-center gap-1.5 rounded-xl border border-border bg-surface-1 px-4 py-2 text-sm font-bold text-foreground hover:bg-surface-2 transition-colors"
                        >
            ...
            {!isEditable && currentConfig && (
                <div className="bg-amber-500/10 border-b border-amber-500/20 px-6 py-3 flex items-center gap-2">
                    <p className="text-sm font-semibold text-amber-600">
                        Đây là phiên bản quá khứ (chỉ đọc). Bạn chỉ có thể chỉnh sửa phiên bản mới nhất.
                    </p>
                </div>
            )}
            ...
            {!currentConfig ? (
                <div className="flex flex-col items-center justify-center py-24 text-center relative overflow-hidden">
                    ...
                    <p className="text-sm text-muted mt-2 max-w-sm relative z-10">
                        Loai san <span className="font-bold text-foreground">{selectedCT.name}</span> chưa được thiết lập giá.
                    </p>
                    {isCourtTypeActive && (
                        <button
                    ...
                    <table className="w-full">
                        <thead>
                            <tr className="border-b border-border">
                                ...
                                <th className="px-6 py-3.5 text-right text-xs font-bold uppercase tracking-wide text-muted">Ngày thường</th>
                                <th className="px-6 py-3.5 text-right text-xs font-bold uppercase tracking-wide text-muted">Cuối tuần</th>
                                <th className="px-6 py-3.5 text-right text-xs font-bold uppercase tracking-wide text-muted">Chênh lệch</th>
                                {currentConfig.rows.some((row) => row.priceSource !== undefined) && (
                                    <th className="px-6 py-3.5 text-center text-xs font-bold uppercase tracking-wide text-muted">Nguồn</th>
                                )}
                                {isEditable && (
                                    <th className="px-6 py-3.5 text-right text-xs font-bold uppercase tracking-wide text-muted">Hành động</th>
                                )}
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-border">
                            {currentConfig.rows.map((row, index) => {
                                ...
                                const hasWeekdayDiff = row.systemWeekdayPrice !== undefined && row.systemWeekdayPrice !== row.weekdayPrice;
                                const hasWeekendDiff = row.systemWeekendPrice !== undefined && row.systemWeekendPrice !== row.weekendPrice;
                                const hasSource = currentConfig.rows.some((sourceRow) => sourceRow.priceSource !== undefined);
                                                ...
                                                <div className="inline-flex items-center justify-end gap-2">
                                                    {isOverride && onResetOverride && (
                                                        <button
                                                            ...
                                                            disabled={!canEdit}
                                                            className={`inline-flex items-center gap-1.5 rounded-xl border border-amber-500/30 bg-amber-500/10 px-3 py-1.5 text-xs font-bold text-amber-600 transition-colors ${!canEdit ? "opacity-50 cursor-not-allowed" : "hover:bg-amber-500/15 opacity-0 group-hover:opacity-100"}`}
                                                        >
                                                    ...
                                                    <button
                                                        ...
                                                        disabled={!canEdit}
                                                        className={`inline-flex items-center gap-1.5 rounded-xl border border-primary/20 bg-primary/5 px-3 py-1.5 text-xs font-bold text-primary transition-colors ${!canEdit ? "opacity-50 cursor-not-allowed" : "hover:bg-primary/10 opacity-0 group-hover:opacity-100"}`}
                                                    >
