    ...
    <div className="grid gap-4 lg:grid-cols-2 xl:grid-cols-3">
        {promotions.map((promotion) => {
            ...
            return (
                <div
                    key={promotion.id}
                    className="group relative flex flex-row items-stretch overflow-hidden rounded-2xl border border-border bg-surface-1 p-2.5 transition-all hover:border-primary/50 hover:shadow-lg hover:shadow-primary/5"
                >
                    {/* Left Side: Image or Ticket Placeholder */}
                    <div className="relative h-28 w-28 flex-shrink-0 overflow-hidden rounded-xl border border-border/50 shadow-inner">
                        {promotion.promoDisplayUrl ? (
                            ...
                            <div className="relative flex h-full w-full flex-col items-center justify-center bg-gradient-to-br from-primary/10 to-primary/5 text-primary/40">
                                ...
                                />
                                <Ticket className="h-10 w-10 mb-1" weight="duotone" />
                    ...
                    <div className="ml-4 flex flex-1 flex-col justify-between py-1">
                        <div className="space-y-1.5">
                            ...
                            <div className="flex items-start justify-between gap-2">
                                ...
                                <Badge variant={statusCfg.variant} size="sm" className="h-5 px-1.5 text-[10px]">
                                    {statusCfg.label}
                                </Badge>
                            </div>
                        ...
                        <div className="mt-3 flex items-center justify-between border-t border-border/40 pt-2">
                            ...
                            {promotion.code ? (
                                <div className="flex items-center gap-1 text-primary">
                                    ...
                                    <span className="font-mono text-xs font-bold tracking-tight">{promotion.code}</span>
                                </div>
                            ) : (
                                <div className="flex items-center gap-1 text-muted-foreground text-[11px]">
                                    <CalendarBlank className="h-3 w-3" />
                                    <span>Hết hạn: {formatDate(promotion.endDate)}</span>
                                </div>
                            )}
                            ...
                            <div className="flex gap-1.5">
                                <button
                                    ...
                                    className="flex h-9 w-9 items-center justify-center rounded-lg hover:bg-primary/10 hover:text-primary text-muted-foreground transition-colors"
                                ...
                                <button
                                    ...
                                    className="flex h-9 w-9 items-center justify-center rounded-lg hover:bg-red-500/10 hover:text-red-500 text-muted-foreground transition-colors"
