...
import {
  ...
  getRevenueGroupBy,
  getGroupByLabel,
} from "@/src/features/dashboard/owner/utils/dashboard-filters";
...
export function OwnerDashboard() {
  ...
  const [branches, setBranches] = useState<
    Array<{ id: string; name: string; status?: 0 | 1 | 2 }>
  >([]);
    ...
    return [
      {
        ...
        sub:
          dashboardData.revenueChangePercent !== null
            ? `${dashboardData.revenueChangePercent > 0 ? "+" : ""}${dashboardData.revenueChangePercent}% so với trước`
            : "Dữ liệu tổng quát",
        trend: (dashboardData.revenueChangePercent === null
          ? "neutral"
      ...
      },
      {
        ...
        sub:
          dashboardData.bookingChangePercent !== null
            ? `${dashboardData.bookingChangePercent > 0 ? "+" : ""}${dashboardData.bookingChangePercent}% so với trước`
            : "Dữ liệu tổng quát",
        trend: (dashboardData.bookingChangePercent === null
          ? "neutral"
          : dashboardData.bookingChangePercent > 0
            ? "up"
            : "down") as "up" | "down" | "neutral",
        icon: Calendar,
      },
      {
        ...
        sub:
          dashboardData.occupancyChangePercent !== null
            ? `${dashboardData.occupancyChangePercent > 0 ? "+" : ""}${dashboardData.occupancyChangePercent}% so với trước`
            : "Tất cả thời gian",
        trend: (dashboardData.occupancyChangePercent === null
          ? "neutral"
          : dashboardData.occupancyChangePercent > 0
            ? "up"
            : "down") as "up" | "down" | "neutral",
        icon: ChartLineUp,
      },
      {
        ...
        sub:
          dashboardData.newCustomerChangePercent !== null
            ? `${dashboardData.newCustomerChangePercent > 0 ? "+" : ""}${dashboardData.newCustomerChangePercent}% so với trước`
            : "Trong kỳ này",
        trend: (dashboardData.newCustomerChangePercent === null
          ? "neutral"
          : dashboardData.newCustomerChangePercent > 0
            ? "up"
            : "down") as "up" | "down" | "neutral",
        icon: Users,
      },
        ...
        value: new Intl.NumberFormat("vi-VN", {
          ...
          Math.round(
            dashboardData.totalRevenue /
              Math.max(dashboardData.completedBookings, 1),
          ),
        ),
  ...
  const revenueLabels =
    data?.revenueTrend
      ...
      .map((i) => formatDisplayPeriod(i.period)) || [];
  ...
  const bookingTrend =
    trendData?.map((i) => {
      ...
      return {
        ...
        label: isHour
          ? `${i.period}h`
          : dayLabels[i.period] || formatDisplayPeriod(i.period),
        isToday: isHour
    ...
    ? [
        {
          ...
          percent:
            Math.round(
              (dashboardData.completedBookings / dashboardData.totalBookings) *
                100,
            ) || 0,
          color: "#1D9E75",
        },
        {
          ...
          percent:
            Math.round(
              (dashboardData.cancelledBookings / dashboardData.totalBookings) *
                100,
            ) || 0,
          color: "#D85A30",
        },
        {
          label: "Vắng mặt",
          value: dashboardData.noShowBookings,
          percent:
            Math.round(
              (dashboardData.noShowBookings / dashboardData.totalBookings) *
                100,
            ) || 0,
          color: "#EF9F27",
        },
  ...
  const onlinePercent = dashboardData
    ? Math.round(
        (dashboardData.onlinePaymentRevenue /
          Math.max(dashboardData.totalRevenue, 1)) *
          100,
      )
    : 0;
  const cashPercent = dashboardData
    ? Math.round(
        (dashboardData.cashPaymentRevenue /
          Math.max(dashboardData.totalRevenue, 1)) *
          100,
      )
    : 0;
    ...
    <div className="space-y-6 animate-slide-up w-full px-8 pt-6 pb-10">
      ...
      <div className="grid gap-6">
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl p-6 shadow-sm">
            <h2 className="text-base font-extrabold text-slate-900 dark:text-white mb-8">
              Xu hướng đặt sân
            </h2>
            {isLoading ? (
              <DashboardChartLoading />
            ) : bookingTrend.length > 0 ? (
              <TrendBarChart data={bookingTrend} height="h-[160px]" />
            ) : (
              <DashboardEmpty
                title="Không có dữ liệu đặt sân"
                icon="calendar"
              />
            )}
          </div>
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl p-6 shadow-sm">
            <h2 className="text-base font-extrabold text-slate-900 dark:text-white mb-6">
              Chi nhánh hàng đầu
            </h2>
                ...
                data?.topBranches.map((branch, idx) => (
                  <LeaderboardRow
                    ...
                    progress={
                      (branch.revenue /
                        Math.max(
                          ...data.topBranches.map((b) => b.revenue),
                          1,
                        )) *
                      100
                    }
                  ...
                  return (
                    <LeaderboardRow
                      ...
                      value={new Intl.NumberFormat("vi-VN", {
                        style: "currency",
                        currency: "VND",
                      }).format(cust.totalRevenue)}
                      subLabel={cust.loyaltyTier}
                      avatarInitials={cust.fullName
                        .split(" ")
                        .map((s) => s[0])
                        .join("")
                        .slice(0, 2)}
