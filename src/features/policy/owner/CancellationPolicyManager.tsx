            ...
            {loading ? (
                <div className="space-y-4 pt-2">
                    {[1, 2, 3].map((i) => (
                        <div
                            key={i}
                            className="h-32 w-full animate-pulse rounded-2xl bg-surface-2 border border-border"
                        />
                    ))}
                </div>
            ) : error ? (
                ...
                <div className="fixed bottom-8 right-8 z-[100] transition-all duration-300 animate-slide-up">
                    ...
                    >
                        {toast.tone === "success" ? (
                            ...
                            <Warning className="h-5 w-5 shrink-0 text-red-500" />
                        )}
                        <p
                            className={`text-sm font-bold ${toast.tone === "success" ? "text-emerald-600" : "text-red-600"
                                }`}
                        >
                            {toast.message}
                        </p>
